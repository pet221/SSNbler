...
ssn_check <- function(ssn.object, check_obs = TRUE, afv_col = NULL, verbose = TRUE) {
    ...
    for(i in 1:length(afv_col)) {
      ...
      if(afv_i %in% colnames(ssn.object$edges)) {
        ...
        if(sum(is.na(edges.df[, afv_i])) > 0) {
          out.message<- paste0(out.message, "edges AFV column, ",
                               afv_i, ", contains NAs\n")
          ##valid<- FALSE
        }
        ...
        out.message<- paste0(out.message, "AFV column, ", afv_i,
                             ", not found in edges\n")
        ##valid <- FALSE
      }
    }      
  ...
  for(p in 1:length(pred.names)) {
    ...
    if(empty.preds > 0) {
      out.message <- paste0(out.message,pred.names[p],
      ...
      for(i in 1:length(afv_col)) {
        ...
        if(afv_i %in% colnames(ssn.object$preds[[pred.names[p]]])) {
          ...
          if(sum(is.na(pred.df[, afv_i])) > 0) {
            out.message<- paste0(out.message, pred.names[p], " AFV column, ",
                                 afv_i, ", contains NAs\n")
            valid<- FALSE
          }
          ...
          out.message<- paste0(out.message, pred.names[p], " AFV column, ", afv_i,
                               ", not found\n")
          valid <- FALSE
        }
      }      
