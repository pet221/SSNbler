...
-   `MF_obs`: An `sf` object with `POINT` geometry containing observed data at 45
...
user. We start with a collection of spatial datasets (e.g.,
geopackages) representing the stream network, observed data, and
prediction data (optional), which are required to create an SSN object
used to fit spatial stream-network models. Geopackages representing
these Middle Fork data are also installed with `SSNbler` and can be
found in the `streamdata` folder. To prevent `SSNbler` functions from
...
the required geometry types. The datasets must all have the same
map projection, which must also be a projected coordinate system, rather
...
prediction sites using the `ggplot2` package.

We load the `ggplot2` package and visualize the stream network, with
prediction and observed locations: 

```{r, fig.cap = "Middle Fork Streams", fig.alt = "Middle Fork Streams", fig.align = "center", out.width = "90%"}
library(ggplot2)
...
spatial stream network model [@peterson2014stars]. A Landscape Network
(LSN) data structure [@theobald2006functional] is used to generate
this information. In a LSN, streams data are represented as a collection of directed `edges`, where
the directionality is determined by the digitized direction of the
line features. Nodes are located at the
end points of edges (i.e., end nodes) and a `nodes` dataset is created when the LSN is
...
```{r, fig.cap = "LSN components are stored in memory as sf objects and also in a local LSN directory as geopackages and csv files, which are accessed using other `SSNbler` functions", fig.alt="LSN Storage", fig.align = "center", echo = FALSE}
...
network) between the lowermost location on a stream network (i.e.,
network outlet) and each feature. For an edge, the distance is
measured to the upstream end node of the line feature. For sites, the
distance is measured between the outlet and the point feature. The
data in `upDist` are used to calculate the directional hydrologic distances
between observed and prediction locations.

The upstream distance for the jth edge, $upDist_j$, is: 
...
found in the path from the network outlet and the jth edge. Note that
...
```
Two columns are added to edge and saved in edges.gpkg. `Length` represents the
length of each edge in map units and `upDist` is the upstream distance for each edge.

The upstream distance for site $i$, $upDist_i$, is calculated a little differently
because it is the hydrologic distance between the network outlet and
each *site*. It is calculated as:
$$
...
* `lsn_path`: The LSN pathname where the edges and sites resides
```{r}
...
influence of one upstream edge over another (e.g. large stream
channel converges with a smaller one) on downstream values. 

Calculating the spatial weights is a three step process: 1)
calculating the segment proportional influence (PI), 2) calculating the
additive function valus (AFVs), and calculating the spatial
weights. Steps 1 and 2 are undertaken in `SSNbler`, while step 3 is
calculated in the package `SSN2` when spatial stream-network models
are fit. 

The PI for each edge, $\omega_j$, is defined as the relative influence
of the jth edge feature on the edge directly downstream. In the
following example, j is based on watershed area, but other simple
measures, such as Shreve's stream order (Shreve 1966), could also be
used as long as a value exists for every line feature in edges (i.e.,
missing data are not allowed). When
two edges, denoted j and k, converge at a node, the PI for the jth
edge is:
...
using the `afv_edges()` function, which generally requires a few
arguments:

* `edges`: An `sf` object representing edges.
* `lsn_path`: The LSN pathname where the edges reside.
* `infl_col`: The name of the numeric column in edges used to measure the
  downstream influence of each feature. Missing values are not allowed.
* `segpi_col`: The name of the new column created in edges that will contain segment PI values.
* `afv_col`: The name of the new column created in edges that will store the AFVs.
...
be 1. If AFVs do not meet this requirement, then an error has occurred.

Once the AFVs have been added to edges, they can
...
their AFVs will be the same. For more on additive function values, see @ver2010moving and
@peterson2010mixed.
...
* `afv_col`: The name of the new column created in each sf object in
  `sites` that will store the AFVs. 
...
```
Each 'sf' dataset in `sites.list` now has an AFV column, which was
generated using watershed area and all AFV values should meet the
...
4. `netIDx.dat files`: one text file for each unique network in edges
   containing information describing the topological relationships
   between edges.
   
When `import = TRUE`, the data stored in the `.ssn` directory are
imported into *R*. The netIDx.dat files are combined behind the
...
@peterson2014stars. The spatial datasets in the `.ssn` directory are
also imported and stored as a `SSN` object in memory when `import =
TRUE`.

The `SSN` object is a list containing four elements:
...
* obs and preds: 
  * `netID` The network identifier value for the edge the site resides
    on
  * `pid`: A unique identifier for each measurement represented as a
...
observed sites, and prediction sites datasets. The information in
these columns are used to keep track of the spatial and topological
relationships in the network. Storing a copy of this critical
information as text in the
`netgeom` column reduces the chances that users will unknowingly make
changes to these data, which in turn could change how relationships are
represented in spatial stream-network
models. 


\textcolor{red}{ADD A PLOT OF THE SSN OBJECT HERE. If possible, with
linewidth in proportion to watershed area}
...
tempearture to elevation and precipitation, with the exponential tailup, spherical
...
*Create a table relating SSNbler functions to STARS tools and openSTARS functions?*
...
# From STARS and openSTARS to SSNbler
