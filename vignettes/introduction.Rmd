...
`MiddleFork04.ssn` used by `SSN2`. `SSNbler` has a few input data sets:
...
Documentation for each data set can be found by running `help()`. For example,
to learn more about `MF_streams`, run `help("MF_streams, package = "SSNbler")`.

While these data sets come with `SSNbler` and can be loaded via `data()`,
we focus here on a more realistic
workflow for the user. We start with a collection of spatial data sets
