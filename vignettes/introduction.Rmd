...
After creating the LSN, it is possible to calculate upstream distance,
which is used to determine flow-connected an flow-unconnected
distances on a stream network that are later required for fitting
spatial stream network models [@peterson2010mixed]. The upstream
distance of edge $j$ is the distance from the stream outlet to the
upper end of the edge, calculated as the sum of the length of all
edges starting from the outlet and moving upstream through edge
$j$. Upstream distance is calculated for each edge using the
...
* `lsn_path`: A path to the directory in which the LSN object is
  stored via `lines_to_lsn()`. `lsn_path` must contain
  `relationships.csv`.
